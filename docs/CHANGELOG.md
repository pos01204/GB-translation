...
### 🛠 Fixed/Improved (개선됨)
- **크롤링 고도화**: Idus(Next.js) 페이지에서 `__NEXT_DATA__`를 활용해 **작품정보(POINT 01~), 옵션, 이미지** 누락을 줄임 (DOM 스크래핑은 폴백)
- **옵션 추출 강화**: 커스텀 콤보박스/리스트박스/모달 형태의 옵션 UI 확장 클릭 로직 보강
- **OCR 안정성 강화**: 상세 이미지 OCR 입력을 **임시 파일 저장 → `genai.upload_file()`** 방식으로 전환하여 추출 성공률 개선
- **성능/비용 제어**: `MAX_OCR_IMAGES` 환경변수로 OCR 처리 장수 제한(기본 20)
